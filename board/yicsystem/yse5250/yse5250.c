 ...
 *
 * Modify :
 *		2012. 07. 30 David yeo in YICSYSTEM Co., Ltd.
 */
...
unsigned int get_board_rev(void)
{
	...
	writel(0x7, PMU_ISP_CONFIGURATION);
	timeout = 1000;
	while ((readl(PMU_ISP_STATUS) & 0x7) != 0x7) {
		if (timeout == 0)
	...
	writel(0x00000031, EXYNOS5_CLKDIV_ISP0);
	writel(0x00000031, EXYNOS5_CLKDIV_ISP1);
	writel(0x00000001, EXYNOS5_CLKDIV_ISP2);
	
	writel(0xDFF000FF, EXYNOS5_CLKGATE_ISP0);
	writel(0x00003007, EXYNOS5_CLKGATE_ISP1);
	...
	rev = (adc_val < YSE5250_REV_0_2_ADC_VALUE/2) ?
	           YSE5250_REV_0_0 : YSE5250_REV_0_2;
	
err_power:
	rev &= YSE5250_REV_MASK;
	pmic = (pmic & YSE5250_REGULATOR_MASK) << YSE5250_REGULATOR_SHIFT;
...
int board_init(void)
{
	...
	/* read ID */
	IIC0_ERead(S5M8767_ADDR, MAX8997_ID, &pmic_id);

	if (pmic_id == 0x77) {
		/* MAX8997 */
		printf("PMIC: MAX8997\n");

		pmic = YSE5250_REGULATOR_MAX8997;
		...
		pmic = YSE5250_REGULATOR_MAX77686;
		...
		printf("G3D: %dmV\t", ((unsigned int)(read_vol_g3d >> 1)* 25) + 600);
		printf("MIF: %dmV\n", ((unsigned int)(read_vol_mif & 0x3F) * 50) + 750);
		printf("MEM: %dmV\n", ((unsigned int)(read_vol_mem & 0x3F) * 50) + 750);

	} else if (pmic_id >= 0x0 && pmic_id <= 0x5) {
		/* S5M8767 */
		printf("PMIC: S5M8767\n");
		pmic = YSE5250_REGULATOR_S5M8767;
	} 
#endif
