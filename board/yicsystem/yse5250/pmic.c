...
void Delay_s(unsigned long loop)
{
	unsigned long i,j;
	for(j=0;j<loop;j++)
		for(i=0;i<DELAY;i++)
...
void pmic8767_init(void)
{
	...
	u8 vdd_arm, vdd_int, vdd_g3d;
	u8 vdd_mif, vdd_mem, vdd_mem_set=0;
	u8 vdd_apll;
	u8 vdd_ldo21, vdd_ldo21_set=0;
	...
	vdd_ldo21 = CALC_S5M8767_LDO1267815_VOLT(CONFIG_SCP_PM_VDD_LDO21);
	...
	DEBUG_LED_ON;
	Delay_s(100000);
	//I2C_S5M8767_VolSetting(0x04, vdd_ldo21, 3);
	IIC0_EWrite(S5M8767_ADDR, 0x5D, 0xDC);
	DEBUG_LED_OFF;
	Delay_s(100000);
	/* BUCK9: VDDF_eMMC 2.8V */
	IIC0_EWrite(S5M8767_ADDR, 0x9A, 0x58);
