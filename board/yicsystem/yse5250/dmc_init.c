...
void mem_ctrl_init_lpddr3(u32 nMEMCLK)
{
	...
	Outp32( 0x10C10000+0x0000, 0x17021A00 );	//- PHY1.CON0.ctrl_atgate=0x0i
	...
	if (pkg_type == POP_TYPE) {
		...
		Outp32( 0x10C00000+0x0008, 0x0011044 );	//- PHY0.CON2.DllDeskewEn=1.
		Outp32( 0x10C10000+0x0008, 0x0011044 );	//- PHY1.CON2.DllDeskewEn=1.
	}	
...
void mem_ctrl_init_ddr3(u32 nMEMCLK)
{
	u32 ap_odt, dram_odt;
	...
	Outp32( 0x10DD0000+0x0010, 0x11100000 );    //- Issue PALL
#endif
	Outp32( 0x10DD0000+0x0004, 0x00302620 );    //- bl[22:20]=8, mem_type[11:8]=7, dsref_en[5]=1, dpwrdn_en[1]=1, cl    k_stop_en[0]=1
